...
- **Networking over the stock CE stack** *(work in progress)* — a universal link shim
  (`mppp.dll`) lets the stock `microstk.exe` + `winsock.dll` run over **Ethernet** instead of
  dial-up PPP. On the Broadband Adapter, **DHCP → DNS → TCP → HTTP** is working, and retail CE
  games (e.g. *4x4 Evolution*) dial out and reach their master server with no patches. DNS
  resolves via DHCP option-6 → the Dreamcast's own flash ISP config → a public resolver. A
  **W5500/MACRAW over SPI** backend is still being brought up.
