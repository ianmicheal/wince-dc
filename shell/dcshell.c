...
static void LoadShortcuts(void)
{
	...
	ShcEntry *e = (ShcEntry *)(buf + 8);
	int got = 0, i, count;
	if (!VmuLoad(buf, sizeof(buf), &got) || got < 8 || hdr[0] != SHC_MAGIC)
		return;
	count = (int)hdr[1];
	for (i = 0; i < count && s_deskN < MAXDESK; i++)
	{
		int idx = e[i].idx;
		if (idx < 0 || idx >= (int)START_N)
			continue;
		if (!s_start[idx].exe && !s_start[idx].path)
			continue;
...
{
	if (s_deskN >= MAXDESK || startIdx < 0 || startIdx >= (int)START_N)
		return 0;
	if (!s_start[startIdx].exe && !s_start[startIdx].path)
	...
	s_deskPos[s_deskN].x = x;
	s_deskPos[s_deskN].y = y;
	s_deskSel = s_deskN;
	s_deskN++;
	SaveShortcuts();
	s_deskDirty = 1;
...
{
	int k;
	if (i < 0 || i >= s_deskN || s_deskUser[i] < 0)
		return;
	for (k = i; k < s_deskN - 1; k++)
	{
		s_desk[k] = s_desk[k + 1];
		s_deskUser[k] = s_deskUser[k + 1];
		s_deskPos[k] = s_deskPos[k + 1];
	}
	s_deskN--;
	if (s_deskSel >= s_deskN)
		s_deskSel = s_deskN - 1;
	if (s_deskSel < 0)
		s_deskSel = 0;
	SaveShortcuts();
...
static void DbgStr(const WCHAR *s)
{
...
static void ShellLaunch(const WCHAR *cmd)
{
	...
	int i;
	for (i = 0; cmd[i] && cmd[i] != L' ' && i < MAX_PATH - 1; i++)
		exe[i] = cmd[i]; // split exe arg
	exe[i] = 0;
	if (cmd[i] == L' ')
		args = cmd + i + 1; // e.g. "dcwplay.exe \CD-ROM\song.mp3"
	if (IsDcwApp(exe))
...
static void DrawWinText(HDC hdc, DcWindow *w, int wi, BOOL active)
{
	...
	GfxText(hdc, w->x + 18, w->y - 16, CL_WHITE, active ? CL_TITLE : RGB(112, 112, 112), g_FontBold,
	        w->title);
...
static void OnKey(WPARAM wp)
{
	...
	if (s_menuOpen)
	{
		if (wp == VK_UP && s_menuSel > 0)
			s_menuSel--;
		if (wp == VK_DOWN && s_menuSel < (int)START_N - 1)
			s_menuSel++;
		if (wp == VK_RETURN)
			ActivateStartItem();
		if (wp == VK_ESCAPE)
	...
	s_dirty = 1;
	{
		...
		if (wp == VK_UP && row > 0)
			s_deskSel--;
		if (wp == VK_DOWN && row < s_deskRows - 1 && s_deskSel + 1 < s_deskN)
			s_deskSel++;
		if (wp == VK_LEFT && s_deskSel - s_deskRows >= 0)
			s_deskSel -= s_deskRows;
		if (wp == VK_RIGHT && s_deskSel + s_deskRows < s_deskN)
			s_deskSel += s_deskRows;
	}
	if (wp == VK_DELETE)
	{
		RemoveShortcut(s_deskSel);
		return;
...
{
	DcWindow *w;
	if (!s_shared || wi < 0 || wi >= DCWIN_MAXWIN || !s_shared->win[wi].inUse)
		return;
	w = &s_shared->win[wi];
	if (s_winMax[wi])
	{
		w->x = s_winRestore[wi][0];
		w->y = s_winRestore[wi][1];
		w->w = s_winRestore[wi][2];
		w->h = s_winRestore[wi][3];
		s_winMax[wi] = 0;
	}
	else
	{
		s_winRestore[wi][0] = w->x;
		s_winRestore[wi][1] = w->y;
		s_winRestore[wi][2] = w->w;
		s_winRestore[wi][3] = w->h;
		w->x = 2;
...
static int HandleClick(int x, int y)
{
	...
	if (s_shared) // windows: topmost (focused) first
	{
		...
		if (s_focus >= 0 && s_shared->win[s_focus].inUse)
			order[n++] = s_focus;
		for (k = 0; k < DCWIN_MAXWIN; k++)
			if (s_shared->win[k].inUse && k != s_focus)
				order[n++] = k;
		for (j = 0; j < n; j++)
		{
	...
	for (k = 0; k < s_deskN; k++) // desktop icons (mutable cell positions)
	{
		...
		if (x >= ix && x < ix + ICELL_W && y >= iy && y < iy + ICELL_H)
		{
			s_deskSel = k;
			if (s_desk[k].path)
				LaunchApp(L"dcwexp.exe", s_desk[k].path);
			else
				ShellLaunch(s_desk[k].exe);
			return 1;
		}
	}
	...
	if (s_shared && !s_menuOpen) // windows, top-most (focused) first
	{
		...
		for (j = 0; j < n; j++)
		{
			...
			if (x >= w->x + w->w - 12 && x <= w->x + w->w + 2 && y >= w->y + w->h - 12 &&
			    y <= w->y + w->h + 2)
			{
				s_dragKind = DRAG_WSIZE;
				s_dragTarget = order[j];
				s_dragOffX = x - (w->x + w->w);
				s_dragOffY = y - (w->y + w->h);
				s_focus = order[j];
				return;
			}
...
static void DropIcon(int target, int x, int y)
{
	...
	if (nx < 0)
		nx = 0;
	if (nx > SCREEN_W - ICELL_W)
		nx = SCREEN_W - ICELL_W;
	if (ny < 0)
		ny = 0;
	if (ny > TASK_Y - ICELL_H)
		ny = TASK_Y - ICELL_H;
	s_deskPos[target].x = nx;
	s_deskPos[target].y = ny;
	s_deskDirty = 1; // icons live in the cached desktop layer
	if (target < s_deskN && s_deskUser[target] >= 0)
		SaveShortcuts();
}
...
static void DragApply(int x, int y)
{
	...
	else if (s_shared && s_dragTarget >= 0 && s_shared->win[s_dragTarget].inUse)
	{
		...
		if (s_dragKind == DRAG_WMOVE)
		{
			int nx = x - s_dragOffX, ny = y - s_dragOffY;
			if (ny < 18)
				ny = 18; // keep the title bar on-screen
			if (nx < -(w->w - 40))
				nx = -(w->w - 40);
			if (nx > SCREEN_W - 40)
				nx = SCREEN_W - 40;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	switch (msg)
	{
		...
		case WM_KEYDOWN:
			if (!s_diKbd)
				OnKey(wp); // fallback when DI keyboard not available
			return 0;
...
static void PublishPointer(int cx, int cy, int down)
{
	...
	if (s_focus >= 0 && s_shared->win[s_focus].inUse && !s_winMin[s_focus])
		order[n++] = s_focus;
	for (k = 0; k < DCWIN_MAXWIN; k++)
		if (s_shared->win[k].inUse && !s_winMin[k] && k != s_focus)
			order[n++] = k;
	for (i = 0; i < n; i++)
	{
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPWSTR lpCmd, int nShow)
{
	...
	for (;;)
	{
		...
		if (DInHasPointer())
		{
			...
			DInCursor(&s_cx, &s_cy);
			if (s_cx != ox || s_cy != oy)
				moved = 1; // cursor moved -> needs a present
		}
		...
		// title bar / window corner / desktop icon and then moves becomes a drag.
		{
			...
			if (ptr && !s_ptrWas) // DOWN: arm a possible drag
			{
				...
				s_downY = s_cy;
				s_dragMoved = 0;
				DragHitTest(s_cx, s_cy);
				s_dirty = 1;
			}
			else if (ptr && s_ptrWas) // HELD: drag once past the threshold
			{
				if (!s_dragMoved &&
				    (abs(s_cx - s_downX) > DRAG_THRESH || abs(s_cy - s_downY) > DRAG_THRESH))
					s_dragMoved = 1;
				if (s_dragMoved && s_dragKind != DRAG_NONE)
					DragApply(s_cx, s_cy);
			}
			else if (!ptr && s_ptrWas) // UP: drop, or click if it never moved
			{
				...
				else if (s_dragKind == DRAG_STARTITEM && s_dragTarget >= 0)
				{
					int h = (int)START_N * ROW_H + 8, my = TASK_Y - h + 4;
					if (s_cy < TASK_Y &&
					    (s_cy < my || s_cx >= 4 + MENU_W)) // dropped on the desktop
		...
		else if (s_dragMoved && s_dragKind == DRAG_STARTITEM && s_dragTarget >= 0 &&
		         s_dragTarget < (int)START_N)
