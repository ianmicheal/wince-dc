...
BOOL CBrowserHost::Create(void)
{
    ...
    if (SUCCEEDED(pUnk->QueryInterface(IID_IOleObject, (void **)&_pOle)))
    {
        ...
        if (SUCCEEDED(_pOle->DoVerb(OLEIVERB_UIACTIVATE, NULL,
                                    (IOleClientSite *)this, 0, _frame, &_rc)))
        {
    ...
    if (_hwnd)
    {
        SetWindowLong(_hwnd, GWL_EXSTYLE,
                      GetWindowLong(_hwnd, GWL_EXSTYLE) | WS_EX_CLIENTEDGE);
        SetFocus(_hwnd);
    }
...
{
    if (_pCP && _cookie) { _pCP->Unadvise(_cookie); _cookie = 0; }
    if (_pCP)   { _pCP->Release();   _pCP   = NULL; }
    if (_pIPAO) { _pIPAO->Release(); _pIPAO = NULL; }
    if (_pIPO)  { _pIPO->Release();  _pIPO  = NULL; }
    if (_pWB2)
    {
        _pWB2->Stop();
        _pWB2->Release();
...
HRESULT CBrowserHost::InitEvents(void)
{
    ...
    if (!_pWB2 ||
        FAILED(_pWB2->QueryInterface(IID_IConnectionPointContainer, (void **)&pCPC)))
        return hr;
...
HRESULT CBrowserHost::LoadUrl(LPCWSTR url)
{
    ...
    if (url[0]==L'd' && url[1]==L'c' && url[2]==L'w') return Navigate(url);   // already dcw://
    rest = url;
    if (url[0]==L'h'&&url[1]==L't'&&url[2]==L't'&&url[3]==L'p'&&url[4]==L':'&&url[5]==L'/'&&url[6]==L'/')
        rest = url + 7;
    lstrcpyW(nav, L"dcw://");
    for (i = 0; rest[i] && i < 590; i++) nav[6 + i] = rest[i];
    nav[6 + i] = 0;
    return Navigate(nav);
}

void CBrowserHost::PumpPending(void) { }   // no BeforeNavigate2 cancel needed anymore

HRESULT CBrowserHost::GoBack(void)    { return _pWB2 ? _pWB2->GoBack()    : E_FAIL; }
HRESULT CBrowserHost::GoForward(void) { return _pWB2 ? _pWB2->GoForward() : E_FAIL; }
HRESULT CBrowserHost::Refresh(void)   { return _pWB2 ? _pWB2->Refresh()   : E_FAIL; }
HRESULT CBrowserHost::Stop(void)      { return _pWB2 ? _pWB2->Stop()      : E_FAIL; }
...
HRESULT CBrowserHost::GetHtmlWindow(IHTMLWindow2 **ppWin)
{
    *ppWin = NULL;
    ...
    if (FAILED(_pWB2->get_Document(&pDisp)) || !pDisp) return E_FAIL;
    IHTMLDocument2 *pDoc = NULL;
    HRESULT hr = pDisp->QueryInterface(IID_IHTMLDocument2, (void **)&pDoc);
    pDisp->Release();
    if (FAILED(hr) || !pDoc) return E_FAIL;
...
HRESULT CBrowserHost::ScrollBy(int dx, int dy)
{
    ...
    if (FAILED(GetHtmlWindow(&pWin)) || !pWin) return E_FAIL;
...
STDMETHODIMP CBrowserHost::QueryInterface(REFIID riid, void **ppv)
{
    ...
    else if (riid == IID_IOleWindow)     *ppv = (IOleWindow *)(IOleInPlaceSite *)this;
    else if (riid == IID_IOleInPlaceSite)*ppv = (IOleInPlaceSite *)this;
    else if (riid == IID_IOleContainer)  *ppv = (IOleContainer *)this;
    else if (riid == IID_IOleCommandTarget) *ppv = (IOleCommandTarget *)this;
    else if (riid == IID_IDispatch)      *ppv = (IDispatch *)(DWebBrowserEvents2 *)this;
    else if (riid == DIID_DWebBrowserEvents2) *ppv = (DWebBrowserEvents2 *)this;
    else { *ppv = NULL; return E_NOINTERFACE; }
    ((IUnknown *)*ppv)->AddRef();
    return S_OK;
}

STDMETHODIMP_(ULONG) CBrowserHost::AddRef(void)  { return ++_refs; }
STDMETHODIMP_(ULONG) CBrowserHost::Release(void)
{
    if (--_refs == 0) { delete this; return 0; }
    return _refs;
}

// ===================== IOleClientSite ====================================
STDMETHODIMP CBrowserHost::SaveObject(void)                       { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::GetMoniker(DWORD, DWORD, IMoniker **p) { if (p) *p = NULL; return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::GetContainer(IOleContainer **pp)       { return QueryInterface(IID_IOleContainer, (void **)pp); }
STDMETHODIMP CBrowserHost::ShowObject(void)                      { return S_OK; }
STDMETHODIMP CBrowserHost::OnShowWindow(BOOL)                    { return S_OK; }
STDMETHODIMP CBrowserHost::RequestNewObjectLayout(void)         { return E_NOTIMPL; }

// ===================== IOleWindow ========================================
STDMETHODIMP CBrowserHost::GetWindow(HWND *p)                    { if (!p) return E_INVALIDARG; *p = _frame; return S_OK; }
STDMETHODIMP CBrowserHost::ContextSensitiveHelp(BOOL)           { return E_NOTIMPL; }

// ===================== IOleInPlaceSite ===================================
STDMETHODIMP CBrowserHost::CanInPlaceActivate(void)            { return S_OK; }
STDMETHODIMP CBrowserHost::OnInPlaceActivate(void)            { return S_OK; }
STDMETHODIMP CBrowserHost::OnUIActivate(void)                { return S_OK; }
STDMETHODIMP CBrowserHost::GetWindowContext(IOleInPlaceFrame **ppFrame,
                                            IOleInPlaceUIWindow **ppDoc,
                                            LPRECT prcPos, LPRECT prcClip,
                                            LPOLEINPLACEFRAMEINFO pFI)
{
    if (ppFrame) *ppFrame = NULL;      // no toolbar/menu negotiation - the host owns chrome
    if (ppDoc)   *ppDoc   = NULL;
    if (prcPos)  *prcPos  = _rc;
    if (prcClip) *prcClip = _rc;
    if (pFI)
    {
...
STDMETHODIMP CBrowserHost::OnInPlaceDeactivate(void)        { return S_OK; }
STDMETHODIMP CBrowserHost::DiscardUndoState(void)          { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::DeactivateAndUndo(void)        { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::OnPosRectChange(LPCRECT)        { return S_OK; }

// ===================== IOleContainer =====================================
STDMETHODIMP CBrowserHost::ParseDisplayName(IBindCtx *, LPOLESTR, ULONG *, IMoniker **) { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::EnumObjects(DWORD, IEnumUnknown **) { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::LockContainer(BOOL)               { return S_OK; }

// ===================== IOleCommandTarget =================================
STDMETHODIMP CBrowserHost::QueryStatus(const GUID *pGroup, ULONG cCmds, OLECMD *rg, OLECMDTEXT *pText)
{
    if (pGroup) return OLECMDERR_E_UNKNOWNGROUP;
    if (pText && pText->cmdtextf != OLECMDTEXTF_NONE) pText->cwActual = 0;
    for (ULONG i = 0; i < cCmds; i++) rg[i].cmdf = 0;
    return S_OK;
}
STDMETHODIMP CBrowserHost::Exec(const GUID *pGroup, DWORD nCmdID, DWORD, VARIANTARG *pIn, VARIANTARG *)
{
    if (pGroup) return OLECMDERR_E_UNKNOWNGROUP;
...
STDMETHODIMP CBrowserHost::GetTypeInfoCount(UINT *p)                 { if (p) *p = 0; return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::GetTypeInfo(UINT, LCID, ITypeInfo **)     { return E_NOTIMPL; }
STDMETHODIMP CBrowserHost::GetIDsOfNames(REFIID, OLECHAR **, UINT, LCID, DISPID *) { return E_NOTIMPL; }

STDMETHODIMP CBrowserHost::Invoke(DISPID dispid, REFIID, LCID, WORD,
                                  DISPPARAMS *pdp, VARIANT *, EXCEPINFO *, UINT *)
{
    ...
    case DISPID_BEFORENAVIGATE2:
    {
        ...
        if (url && (url[0] == L'h' || url[0] == L'H')) { UiOnNavigate(url); UiOnBusy(1); }
        break;
    }

    case DISPID_NAVIGATECOMPLETE2:
    case DISPID_DOCUMENTCOMPLETE:
        // Don't surface the URL here: our internal navigations are about:blank / file://\dcwie.htm,
        // and LoadUrl already put the real address in the bar. Just clear the busy flag.
        if (dispid == DISPID_DOCUMENTCOMPLETE) { OutputDebugStringW(L"IE evt: DocumentComplete\r\n"); UiOnBusy(0); }
        break;

    case DISPID_DOWNLOADBEGIN:   OutputDebugStringW(L"IE evt: DownloadBegin\r\n");    UiOnBusy(1); break;
    case DISPID_DOWNLOADCOMPLETE: OutputDebugStringW(L"IE evt: DownloadComplete\r\n"); UiOnBusy(0); break;
    ...
    case DISPID_SETSECURELOCKICON:
        ...
        {
            int sec = 0;
            if (a[0].vt == VT_I4)        sec = (a[0].lVal > 0);
            else if (a[0].vt == VT_BOOL) sec = (a[0].boolVal != 0);
            UiOnSecure(sec);
        }
        break;
#endif
