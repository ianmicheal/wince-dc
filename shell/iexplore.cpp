...
extern "C" void UiOnNavigate(const WCHAR *url)
{
	if (url)
		StrCpyN(g_url, url, 512);
}
extern "C" void UiOnTitle(const WCHAR *title)
{
	if (title)
		StrCpyN(g_title, title, 256);
}
...
extern "C" void UiOnBusy(int busy)
{
	g_busy = busy;
}
extern "C" void UiOnSecure(int sec)
{
	g_secure = sec;
}
...
#define KORG_Y     150
#define KSP_W      100
static int KbRowCount(int row)
{
	return (row == KB_SPECROW) ? KB_SPECIAL : KB_COLS;
}
...
static void RenderKeyboard(void)
{
	...
	for (col = 0; col < KB_SPECIAL; col++)
	{
		int sel = (g_selRow == KB_SPECROW && col == g_selCol);
		int x = KORG_X + col * (KSP_W + 2), y = KORG_Y + KB_CHARROWS * (KCELL_H + 2);
		GfxText(dc, x + 6, y + 8, sel ? CL_WHITE : CL_TEXT, sel ? CL_SEL : CL_ADDR, g_FontUI,
		        kbSpecial[col]);
	}
	GfxUnlockDC(dc);
...
static void LeaveUrlMode(int navigate)
{
	...
	if (navigate && g_edit[0] && g_host)
	{
		...
		if (hasScheme)
			StrCpyN(url, g_edit, 540);
		else
		{
			lstrcpyW(url, L"http://");
			StrCpyN(url + 7, g_edit, 533);
		}
		StrCpyN(g_url, url, 512);
		g_busy = 1;
...
static void KbMove(UINT vk)
{
	...
	if (vk == VK_UP)
		g_selRow = (g_selRow + rows - 1) % rows;
	if (vk == VK_DOWN)
		g_selRow = (g_selRow + 1) % rows;
	n = KbRowCount(g_selRow);
	if (vk == VK_LEFT)
		g_selCol = (g_selCol + n - 1) % n;
	if (vk == VK_RIGHT)
		g_selCol = (g_selCol + 1) % n;
	if (g_selCol >= n)
		g_selCol = n - 1;
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPWSTR lpCmd, int nShow)
{
	...
	g_frame = CreateWindowExW(0, L"DCIEXPLORE", L"Internet Explorer",
	                          WS_POPUP | WS_VISIBLE | WS_CLIPCHILDREN, 0, 0, SCRW, SCRH, NULL, NULL,
	                          hInst, NULL);
	if (!g_frame)
	{
		CoUninitialize();
	...
	while (g_run)
	{
		while (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			if (g_host && !g_urlMode) // let the control handle its own keyboard msgs
			{
				...
				if (cw && (msg.hwnd == cw || IsChild(cw, msg.hwnd)) && msg.message >= WM_KEYFIRST &&
				    msg.message <= WM_KEYLAST && g_host->TranslateAccel(&msg) == S_OK)
					continue;
			}
		...
		if (g_urlMode)
			...
			PollPageMode();
		if (g_host)
			g_host->PumpPending(); // winsock-load any link the control tried to open
