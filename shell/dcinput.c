...
static int g_mHeld = 0,
           g_jHeld = 0; // pointer button currently HELD (mouse-L / controller-A) for drag
...
static BOOL CALLBACK JoyObjCb(LPCDIDEVICEOBJECTINSTANCE o, LPVOID ctx)
{
	...
	if (LOBYTE(LOWORD(o->dwType)) & DIDFT_BUTTON)
	{
		...
		if (u >= 0 && u < USG_N)
			g_btnIdx[u] = (signed char)g_btnEnum;
		g_btnEnum++;
	}
	...
	// have, so the 1s hotplug re-scan doesn't spam; we still see a mouse the moment it appears.
	{
		...
		if (!have)
		{
			...
			wsprintfW(b, L"DCIN: enum type=%u (2=mouse 3=kbd 4=joy) dwDevType=%08x\r\n",
			          (unsigned)t, (unsigned)di->dwDevType);
	...
	else if (t == DIDEVTYPE_JOYSTICK && !g_joy)
	{
		...
		g_joy = d2;
		{
			...
			wsprintfW(b, L"DCIN: joystick acquired, %d buttons (A@%d B@%d X@%d Start@%d)\r\n",
			          g_btnEnum, g_btnIdx[USG_A], g_btnIdx[USG_B], g_btnIdx[USG_X],
			          g_btnIdx[USG_START]);
			OutputDebugStringW(b);
		}
	}
...
void DInShutdown(void)
{
	if (g_kbd)
	{
		IDirectInputDevice2_Unacquire(g_kbd);
		IDirectInputDevice2_Release(g_kbd);
		g_kbd = NULL;
	}
	if (g_mouse)
	{
		IDirectInputDevice2_Unacquire(g_mouse);
		IDirectInputDevice2_Release(g_mouse);
		g_mouse = NULL;
	}
	if (g_joy)
	{
		IDirectInputDevice2_Unacquire(g_joy);
		IDirectInputDevice2_Release(g_joy);
		g_joy = NULL;
	}
	if (g_di)
	{
		IDirectInput_Release(g_di);
		g_di = NULL;
	}
}
...
void DInReacquire(void)
{
	...
	g_joyPrimeUntil = GetTickCount() + JOY_PRIME_MS;
	g_mousePrimed = 0;
	g_qh = g_qt = 0; // drop any queued keys from before the hand-off
	memset(g_beEdge, 0, sizeof(g_beEdge));
	memset(g_beHeld, 0, sizeof(g_beHeld));
...
void DInUpdate(void)
{
	...
	if (g_joy)
	{
		...
		else
		{
			...
			if (dt > 100)
				dt = 100; // clamp after a stall so the cursor doesn't leap
			// analog stick -> cursor: time-based + sub-pixel accumulator (speed is
			// frame-rate independent AND small deflections still move)
			if (ax > STICK_DZ)
			{
				g_accX += (long)js.lX * (long)dt;
				dx = (int)(g_accX / STICK_DIV);
				g_accX -= (long)dx * STICK_DIV;
				g_cx += dx;
			}
			else
				g_accX = 0;
			if (ay > STICK_DZ)
			{
				g_accY += (long)js.lY * (long)dt;
				dy = (int)(g_accY / STICK_DIV);
				g_accY -= (long)dy * STICK_DIV;
				g_cy += dy;
			}
			else
				g_accY = 0;
			...
			else
			{
				...
				for (i = 0; i < 4; i++)
				{
					if (dn[i] && !g_dpadHeld[i])
					{
						Push(s_dpadVk[i]);
						g_dpadRepeatAt[i] = nowt + 350;
					}
					else if (dn[i] && nowt >= g_dpadRepeatAt[i])
					{
						Push(s_dpadVk[i]);
						g_dpadRepeatAt[i] = nowt + 120;
					}
					g_dpadHeld[i] = dn[i];
				}
...
{
	return (g_mouse != NULL) || (g_joy != NULL) || g_wmPointer;
}
