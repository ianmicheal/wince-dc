...
{ int i = 0; while (src[i]) { dst[i] = src[i]; i++; } return i; }
static const char *StrStrA2(const char *hay, const char *ndl)
{ int i, j; for (i = 0; hay[i]; i++) { for (j = 0; ndl[j] && hay[i+j] == ndl[j]; j++) ; if (!ndl[j]) return hay + i; } return 0; }

static char *HttpFetch(const char *host, int port, const char *path, int *outLen)
{
    SOCKET s = INVALID_SOCKET; struct hostent *he; SOCKADDR_IN sa;
    char *buf = NULL; int len = 0, n, rl; char req[700];
    __try
    {
        ...
        if (connect(s, (SOCKADDR *)&sa, sizeof(sa)) != 0) { closesocket(s); return NULL; }
        rl  = AppendA(req, "GET ");
        ...
        for (;;) { if (len >= HTTP_MAXRESP - 4096) break; n = recv(s, buf + len, 4096, 0); if (n <= 0) break; len += n; }
        closesocket(s);
        buf[len] = 0; *outLen = len; return buf;
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    { if (s != INVALID_SOCKET) closesocket(s); if (buf) LocalFree(buf); return NULL; }
}
...
{
    char url[600]; int hop, i = 0;
    while (urlIn[i] && i < 599) { url[i] = urlIn[i]; i++; } url[i] = 0;

    for (hop = 0; hop < 4; hop++)
    {
        const char *u = url, *h; char host[256], path[512]; int port = 80, k;
        if (u[0]=='h'&&u[1]=='t'&&u[2]=='t'&&u[3]=='p'&&u[4]=='s') return NULL;   // no TLS
        if (u[0]=='d'&&u[1]=='c'&&u[2]=='w'&&u[3]==':'&&u[4]=='/'&&u[5]=='/') u += 6;        // our scheme
        else if (u[0]=='h'&&u[1]=='t'&&u[2]=='t'&&u[3]=='p'&&u[4]==':'&&u[5]=='/'&&u[6]=='/') u += 7;
        for (k = 0; u[k] && u[k] != '/' && u[k] != ':' && k < 255; k++) host[k] = u[k];
        host[k] = 0; h = u + k;
        if (*h == ':') { port = 0; h++; while (*h >= '0' && *h <= '9') port = port*10 + (*h++ - '0'); }
        if (*h != '/') { path[0] = '/'; path[1] = 0; }
        else { k = 0; while (h[k] && k < 510) { path[k] = h[k]; k++; } path[k] = 0; }

        { int len; char *resp = HttpFetch(host, port, path, &len);
          if (!resp) return NULL;
          if (resp[9] == '3')                                   // redirect
          {
              const char *loc = StrStrA2(resp, "\nLocation:");
              if (!loc) loc = StrStrA2(resp, "\nlocation:");
              if (loc) { int j = 0; loc += 10; while (*loc == ' ') loc++;
                         while (*loc && *loc!='\r' && *loc!='\n' && j < 599) url[j++] = *loc++;
                         url[j] = 0; LocalFree(resp); continue; }
          }
...
class CDcwHttp : public IInternetProtocol
{
    ...
    int   _resultDone;
public:
    CDcwHttp() { _refs = 1; _resp = NULL; _bodyOff = _bodyLen = _pos = 0; _pSink = NULL; _resultDone = 0; }
    ~CDcwHttp() { if (_resp) LocalFree(_resp); if (_pSink) _pSink->Release(); }

    STDMETHOD(QueryInterface)(REFIID riid, void **ppv)
    {
        if (riid == IID_IUnknown || riid == IID_IInternetProtocolRoot || riid == IID_IInternetProtocol)
        { *ppv = (IInternetProtocol *)this; AddRef(); return S_OK; }
        *ppv = NULL; return E_NOINTERFACE;
    }
    STDMETHOD_(ULONG, AddRef)(void)  { return ++_refs; }
    STDMETHOD_(ULONG, Release)(void) { if (--_refs == 0) { delete this; return 0; } return _refs; }
    ...
    {
        char urlA[600];
        (void)pBI; (void)grfPI; (void)dwReserved;
        WideCharToMultiByte(CP_ACP, 0, szUrl, -1, urlA, sizeof(urlA), NULL, NULL);
        OutputDebugStringW(L"proto: Start "); OutputDebugStringW(szUrl); OutputDebugStringW(L"\r\n");
        ...
        { WCHAR b[64]; wsprintfW(b, L"proto: %d bytes\r\n", _bodyLen); OutputDebugStringW(b); }
        _pSink = pSink; _pSink->AddRef();        // ReportResult is deferred until Read drains
        ...
        pSink->ReportData(BSCF_FIRSTDATANOTIFICATION | BSCF_LASTDATANOTIFICATION | BSCF_DATAFULLYAVAILABLE,
                          _bodyLen, _bodyLen);
        return S_OK;
    }
    STDMETHOD(Continue)(PROTOCOLDATA *)        { return S_OK; }
    STDMETHOD(Abort)(HRESULT, DWORD)           { return S_OK; }
    STDMETHOD(Terminate)(DWORD)                { if (_pSink) { _pSink->Release(); _pSink = NULL; } return S_OK; }
    STDMETHOD(Suspend)(void)                   { return E_NOTIMPL; }
    STDMETHOD(Resume)(void)                    { return E_NOTIMPL; }

    // IInternetProtocol
    STDMETHOD(Read)(void *pv, ULONG cb, ULONG *pcbRead)
    {
        ULONG avail, nn;
        if (!_resp) { if (pcbRead) *pcbRead = 0; return S_FALSE; }
        avail = (ULONG)(_bodyLen - _pos);
        nn = (cb < avail) ? cb : avail;
        if (nn) memcpy(pv, _resp + _bodyOff + _pos, nn);
        _pos += (int)nn;
        if (pcbRead) *pcbRead = nn;
        if (_pos >= _bodyLen)                    // drained -> NOW signal the bind completed
        {
            if (_pSink && !_resultDone) { _resultDone = 1; _pSink->ReportResult(S_OK, 0, NULL); }
            return S_FALSE;
        }
        return S_OK;
    }
...
public:
    STDMETHOD(QueryInterface)(REFIID riid, void **ppv)
    {
        ...
        { *ppv = (IClassFactory *)this; return S_OK; }
        *ppv = NULL; return E_NOINTERFACE;
    }
    STDMETHOD_(ULONG, AddRef)(void)  { return 2; }     // never freed
...
extern "C" void IeRegisterWinsockProtocol(void)
{
    ...
    if (CoInternetGetSession(0, &pSess, 0) == S_OK && pSess)
    {
        ...
        pSess->Release();
        { WCHAR b[64]; wsprintfW(b, L"IE: register http handler hr=%08x\r\n", (unsigned)hr); OutputDebugStringW(b); }
    }
    else OutputDebugStringW(L"IE: CoInternetGetSession FAILED\r\n");
}
