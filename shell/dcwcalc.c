...
static double ParseNum(const WCHAR *s)
{
	...
	for (; s[i]; i++)
	{
		...
		if (s[i] < L'0' || s[i] > L'9')
			break;
		if (!frac)
			v = v * 10.0 + (double)(s[i] - L'0');
		else
		{
			scale *= 0.1;
			v += (double)(s[i] - L'0') * scale;
		}
	}
	return neg ? -v : v;
}
...
static void FmtNum(double v, WCHAR *out)
{
	...
	for (k = 5; k >= 0; k--)
	{
		f[k] = (WCHAR)(L'0' + frac % 10);
		frac /= 10;
	}
	f[6] = 0;
	fdig = 6;
	while (fdig > 0 && f[fdig - 1] == L'0')
		fdig--;
	f[fdig] = 0;
...
static int HandleKey(DWORD k)
{
	...
	{
		InputDigit((WCHAR)(L'0' + (k - VK_NUMPAD0)));
		return 1;
	}
	switch (k)
	{
