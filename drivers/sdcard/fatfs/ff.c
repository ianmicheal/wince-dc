...
#endif
		rs = disk_read(fs->drv, fs->win, sector, 1);
...
#if _USE_FASTSEEK    /* references FIL.cltbl, which only exists with fastseek; calls are gated too */
	...
	if (!rs) {
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (fs_dma_enabled() == FS_DMA_STREAM && part < SS(fp->fs)) {
					rcnt += part;
				}
