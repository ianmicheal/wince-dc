...
DWORD AfdRasDial(void *ext, const WCHAR *phonebook, void *params, DWORD notifierType,
                 void *notifier, HRASCONN *phConn)
{
...
{
	if (g_useDial && g_dialRas[8])
		return ((FnHangUp)g_dialRas[8])(h);
	(void)h;
	g_dialed = 0;
	return 0;
}
...
{
	if (g_useDial && g_dialRas[3])
		return ((FnEnumConn)g_dialRas[3])(conns, cb, count);
	(void)conns;
	(void)cb;
	if (count)
		*count = g_dialed ? 1 : 0;
...
{
	if (g_useDial && g_dialRas[5])
		return ((FnConnStat)g_dialRas[5])(h, st);
	(void)h;
	if (st)
	{
		st->rasconnstate = g_dialed ? RASCS_Connected : RASCS_Disconnected;
		st->dwError = 0;
	}
	return 0;
}
...
static void EnsureDefaultEntry(void)
{
	...
	if (OpenBook(KEY_READ, &h) == ERROR_SUCCESS)
	{
		WCHAR nm[64];
		DWORD nl = 64;
		if (RegEnumKeyExW(h, 0, nm, &nl, 0, 0, 0, 0) == ERROR_SUCCESS)
			have = 1;
		RegCloseKey(h);
	}
	if (!have && OpenEntry(L"Desktop", TRUE, &e) == ERROR_SUCCESS)
		RegCloseKey(e);
}
...
static void ReadVal(HKEY h, const WCHAR *v, void *out, int cb)
{
	DWORD t, n = cb;
	((BYTE *)out)[0] = 0;
	RegQueryValueExW(h, v, 0, &t, (BYTE *)out, &n);
}
...
DWORD AfdRasEnumEntries(void *a, void *b, RASENTRYNAME *entries, DWORD *cb, DWORD *count)
{
	HKEY h;
	DWORD i = 0, got = 0;
	if (g_useDial && g_dialRas[4])
		return ((FnEnumEnt)g_dialRas[4])(a, b, entries, cb, count);
	(void)a;
	(void)b;
	if (count)
		*count = 0;
	...
	for (;;)
	{
		WCHAR nm[64];
		DWORD nl = 64;
		if (RegEnumKeyExW(h, i, nm, &nl, 0, 0, 0, 0) != ERROR_SUCCESS)
			break;
...
{
	HKEY h;
	if (g_useDial && g_dialRas[6])
		return ((FnGetDial)g_dialRas[6])(reserved, p, pw);
	(void)reserved;
	if (pw)
		*pw = FALSE;
	if (!p)
...
{
	HKEY h;
	if (g_useDial && g_dialRas[11])
		return ((FnSetDial)g_dialRas[11])(reserved, p, del);
	(void)reserved;
	(void)del;
	if (!p)
		return ERROR_INVALID_PARAMETER;
	if (OpenEntry(p->szEntryName, TRUE, &h) != ERROR_SUCCESS)
		return ERROR_ACCESS_DENIED;
	RegSetValueExW(h, L"User", 0, REG_SZ, (const BYTE *)p->szUserName,
	               (lstrlenW(p->szUserName) + 1) * sizeof(WCHAR));
	RegSetValueExW(h, L"Domain", 0, REG_SZ, (const BYTE *)p->szDomain,
	               (lstrlenW(p->szDomain) + 1) * sizeof(WCHAR));
	RegSetValueExW(h, L"Password", 0, REG_BINARY, (const BYTE *)p->szPassword,
	               (lstrlenW(p->szPassword) + 1) * sizeof(WCHAR));
	RegCloseKey(h);
	return 0;
}

DWORD AfdRasSetEntryProperties(void *reserved, const WCHAR *e, void *props, DWORD cb, void *dev,
                               DWORD dcb)
{
	HKEY h;
	if (g_useDial && g_dialRas[12])
		return ((FnSetProp)g_dialRas[12])(reserved, e, props, cb, dev, dcb);
	(void)reserved;
	(void)dev;
	(void)dcb;
	if (OpenEntry(e, TRUE, &h) != ERROR_SUCCESS)
		return ERROR_ACCESS_DENIED;
	if (props && cb)
		RegSetValueExW(h, L"Entry", 0, REG_BINARY, (const BYTE *)props, cb);
	RegCloseKey(h);
	return 0;
}

DWORD AfdRasGetEntryProperties(void *reserved, const WCHAR *e, void *props, DWORD *cb, void *dev,
                               DWORD *dcb)
{
	HKEY h;
	DWORD t;
	if (g_useDial && g_dialRas[7])
		return ((FnGetProp)g_dialRas[7])(reserved, e, props, cb, dev, dcb);
	(void)reserved;
	(void)dev;
	if (dcb)
		*dcb = 0;
	if (OpenEntry(e, FALSE, &h) != ERROR_SUCCESS)
	{
		if (cb)
			*cb = 0;
		return 0;
	...
	if (cb && props)
		RegQueryValueExW(h, L"Entry", 0, &t, (BYTE *)props, cb);
	RegCloseKey(h);
	return 0;
}
...
{
	HKEY h;
	if (g_useDial && g_dialRas[13])
		return ((FnValidate)g_dialRas[13])(reserved, e);
	(void)reserved;
	if (OpenEntry(e, FALSE, &h) == ERROR_SUCCESS)
	{
		RegCloseKey(h);
...
{
	WCHAR path[96];
	if (g_useDial && g_dialRas[1])
		return ((FnDelete)g_dialRas[1])(reserved, e);
	(void)reserved;
	lstrcpyW(path, RASBOOK);
	lstrcatW(path, L"\\");
	lstrcatW(path, e ? e : L"");
...
{
	if (g_useDial && g_dialRas[10])
		return ((FnRename)g_dialRas[10])(reserved, o, n);
	(void)reserved;
	(void)o;
	(void)n;
	return 0;
...
{
	if (g_useDial && g_dialRas[9])
		return ((FnIoctl)g_dialRas[9])(h, code, in, inl, out, outl);
	(void)h;
	(void)code;
	(void)in;
	(void)inl;
